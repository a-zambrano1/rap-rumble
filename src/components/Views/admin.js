  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      if (deleted.message === "member deleted") {
        ...
        notify(
          "error",
          "No se ha podido eliminar el miembro. Inténtalo de nuevo."
        );
      }
    } catch (error) {
      console.error(error);
    ...
    >
      <div className="flex flex-col min-w-[25%] max-w-[90%] gap-9 my-auto py-7 items-center border-4 rounded-3xl border-black bg-white">
        ...
        <section className="flex flex-col items-center w-full px-2 gap-5">
          ...
          <div className="justify-center items-center overflow-auto w-full">
            <h1 className="text-[30px] text-[#3d405b] text-center">
              Miembros de UdeRap
            </h1>
            <hr class="w-4/5 h-0.5 bg-[#000000] m-auto my-2" />
            <div
              className="flex flex-col h-96 w-full overflow-auto"
            >
              {members != null ? (
                      ...
                      .map((member, index) => (
                        <tr key={index} className="text-center">
                          ...
                          <td className="flex justify-center p-2 gap-3">
                            <button
                              className="bg-verdesito hover:bg-verde text-white font-bold py-2 px-2 rounded"
                              onClick={() => {
            ...
            >
              ¿Estás seguro que deseas eliminar a este miembro de la
              competencia?
            </ModalDelete>
            <hr class="w-4/5 h-0.5 bg-[#000000] m-auto my-2" />
            <div className="flex gap-5 w-full justify-around">
              <button
                ...
                className="flex items-center rounded-3xl hover:bg-verdesito bg-verde text-white w-auto h-auto p-3"
              >
                Añadir Miembros
              </button>
              <ModalAdmin
              ...
              <button
                onClick={logout}
                className="rounded-3xl bg-verde hover:bg-verdesito text-white p-3 w-auto h-auto"
              >
